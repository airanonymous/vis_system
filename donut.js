...
(() => {
  ...
  function init(container){
      ...
      .selectAll("span.leg").data(EMOS).enter().append("span")
        ...
        .on("click", function(_, emo){
          ...
          d3.select(this).select(".swatch")
                          .style("opacity", hidden[emo] ? 0.25 : 1);
          draw();
        });
    ...
    d3.csv(FILE, d3.autoType).then(data => {
      rows = data.map(d => {
        // Ses01F --> F01   (fallback keeps original gender+session)
        const m  = d.path.match(/Ses(\d{2})([FM])/);
        const id = m ? `${m[2]}${m[1]}` :
                       `${d.gender}${String(d.session).padStart(2,"0")}`;
        d.speaker = id;
        return d;
      });
  ...
  function draw(){
    ...
    const dataSubset = id === "all" ? rows
                                    : rows.filter(d => d.speaker === id);

    /* roll up counts by emotion */
    const counts = d3.rollups(dataSubset,
                              v => v.length,
                              d => d.emotion)
    ...
    const slices = svg.selectAll("path.slice")
                      .data(pie(data), d => d.data.emotion);
    ...
    /* ------- LABELS ON RING ------- */
    const lbl = svg.selectAll("text.lbl")
                   .data(pie(data), d => d.data.emotion);
    ...
    // word “utterances” underneath
    svg.selectAll("text.totalLabel").data([null])
       .join("text")
